...
按时间周期交替遮蔽船身一侧的红外传感器，每个遮蔽段之间留有短暂的不遮蔽间隔，让船能对准下一个门：

```
Cycle 0 (0~8s):   遮蔽右侧 (IRM_2/3/4) — 只响应左侧的红外信号
Cycle 0 (8~10s):  全部解除 (--)     — 间隙期，全传感器对准门
Cycle 1 (10~18s): 遮蔽左侧 (IRM_6/7/8) — 只响应右侧的红外信号
Cycle 1 (18~20s): 全部解除 (--)     — 间隙期
循环往复...
```
...
| `MASK_UNMASK_GAP_MS` | 2000 | 遮蔽切换间的不遮蔽间隔（毫秒），用于船对准下一个门 |
| `MASK_FIRST_RIGHT` | 1 | 1=先遮蔽右侧再交替, 0=先遮蔽左侧再交替（根据赛道S弯方向修改） |

#### OLED 状态显示

底行新增 Cycle 和遮蔽状态：`W:+9 A:77 C2 MR`
- `C2` = 当前处于 Cycle 2
- `MR` = 遮蔽右侧（Mask Right），`ML` = 遮蔽左侧（Mask Left），`--` = 间隙期无遮蔽
